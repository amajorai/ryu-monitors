...
//!
//! - **Spider fetch** (`mcp_call_tool`) → a Core kernel capability (`POST
//!   /api/host/capability/mcp.callTool`, ext-bearer authed, gated on the
//!   `tools.invoke` grant this manifest declares in `sidecars[].host_api.grants`):
//!   the Spider crawler needs Core's `McpRegistry`, which the sidecar does not host,
//!   so Core runs the tool on the sidecar's behalf and returns the JSON.
...
//! - **alert fan-out** (`MonitorNotifier::deliver`) → a Core kernel capability (`POST
//!   /api/host/capability/notify.fanout`, ext-bearer authed): the kernel notification store +
...
/// Kept identical in `monitors.manifest.json`.
const DEFAULT_PORT: u16 = 8003;

/// The built-in Monitors app id (matches the `monitors.manifest.json` fixture id and
...
async fn main() -> anyhow::Result<()> {
    ...
    // endpoint is what makes the whole `/api/monitors` surface callable by an agent
    // (`ryu_monitors::api::openapi()` was dead code until now — only tests read it).
    //
    // Root, not under `/api/monitors`: Core tries the root FIRST and only falls back
    ...
    // an app surface. Inside the gate, not next to the un-gated `/health`: Core stamps
    ...
    let monitors = Router::new()
        ...
        .route(
            "/openapi.json",
            get(|| async { Json(ryu_monitors::api::openapi()) }),
        )
...
impl ryu_monitors::MonitorsHost for CoreCallback {
    async fn mcp_call_tool(
        ...
        let out = self.post("/api/host/capability/mcp.callTool", body).await?;
...
impl ryu_monitors::MonitorNotifier for CoreCallback {
    async fn deliver(&self, alert: &ryu_monitors::Alert, targets: &[NotifyTarget]) {
        ...
        if let Err(e) = self.post("/api/host/capability/notify.fanout", body).await {
...
#[cfg(test)]
mod tests {
    use super::{bearer_ok, health, looks_like_cron, CoreCallback};
    use ryu_monitors::{MonitorStore, MonitorsHost};

    fn temp_store() -> MonitorStore {
        let path = std::env::temp_dir().join(format!(
            "ryu-monitors-main-test-{}.db",
    ...
    fn ct_eq_compares_content_not_prefix() {
        ...
        assert!(!ryu_sidecar_runtime::constant_time_eq(b"ab", b"abc")); // length mismatch short-circuits
    ...
    fn cron_sanity_accepts_five_fields_rejects_prose() {
        ...
        assert!(!looks_like_cron("a b c d e")); // non-cron chars
    }

    #[tokio::test]
    async fn health_reports_ok_and_monitor_count() {
        let store = temp_store();
        let resp = health(store).await;
        assert_eq!(resp.status(), axum::http::StatusCode::OK);
    }

    #[test]
    fn core_callback_stub_methods_are_inert() {
        // Constructing the callback reads env with defaults; no network happens
        // until a method actually POSTs, and the stub methods never do.
        let cb = CoreCallback::new();
        assert!(cb.sync_backing_job("mon_1", "n", "5m", true).is_ok());
        cb.remove_backing_job("mon_1"); // no panic
                                        // interval validation is a pure local check (humantime OR cron).
        assert!(cb.interval_is_valid("15m"));
        assert!(cb.interval_is_valid("*/5 * * * *"));
        assert!(!cb.interval_is_valid("not-a-schedule"));
    }
}
