...
pub(crate) async fn guarded_fetch_text(url: &str) -> Result<(u16, String), String> {
    ...
    Err(format!(
        "too many redirects (more than {MAX_REDIRECT_HOPS})"
    ))
}
...
mod tests {
    ...
    async fn screen_url_rejects_bad_scheme_and_internal_hosts() {
        ...
        assert!(screen_url("http://169.254.169.254/latest/meta-data/")
            .await
            .is_err());
    ...
    async fn guarded_fetch_rejects_internal_targets() {
        ...
        assert!(guarded_fetch_text("http://169.254.169.254/latest/")
            .await
            .is_err());
